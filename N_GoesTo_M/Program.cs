    ...
    internal class Program
    {
        ...
        static List<int> SolveUsingBFS(int n, int m)
        {
            ...
            while (dp[^1] == -1)
            {
                ...
                int[] options = new[] { current + 1, current + 2, current * 2 };
        ...
        static List<int> SolveIteratively(int n, int m)
        {
            ...
            //dp[m - n] = new List<int>(capacity: 1) { m };
            dp[^1] = new List<int>(capacity: 1) { m };
            for (int i = m - 1; i >= n; i--)
            {
                int[] options = new int[] {i + 1, i + 2, i * 2};
                ...
                for (int j = 0; j < options.Length; j++)
                {
                    ...
                    List<int> preComputedPathForOption = dp[^(m - options[j] - n)];
